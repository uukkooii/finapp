...
class DatabaseHelper {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 2) {
      try {
        await db.execute('ALTER TABLE goals ADD COLUMN account TEXT');
      } catch (_) {}
      try {
  ...
  Future<List<Map<String, dynamic>>> getBudgetsByMonth(String month) async {
    final db = await database;
    return db.query('budgets', where: 'month = ?', whereArgs: [month]);
  }
  ...
  Future<int> upsertBudget(Map<String, dynamic> row) async {
    ...
    final existing = await db.query('budgets',
        ...
        whereArgs: [row['category'], row['month']]);
    if (existing.isEmpty) {
  ...
  Future<List<Map<String, dynamic>>> getGoals() async {
    final db = await database;
    return db.query('goals', orderBy: 'created_at DESC');
  }

  Future<int> updateGoal(Map<String, dynamic> row) async {
    final db = await database;
    return db.update('goals', row, where: 'id = ?', whereArgs: [row['id']]);
  }
  ...
  Future<int> insertBill(Map<String, dynamic> row) async {
    final db = await database;
    return db.insert('bills', row);
  }

  Future<List<Map<String, dynamic>>> getBills() async {
    final db = await database;
    return db.query('bills', where: 'is_active = 1', orderBy: 'due_day ASC');
  }
  ...
  Future<int> insertRecurringBill(Map<String, dynamic> row) async {
    final db = await database;
    return db.insert('recurring_bills', row);
  }

  Future<List<Map<String, dynamic>>> getRecurringBills() async {
    final db = await database;
    return db.query('recurring_bills', orderBy: 'next_due_date ASC');
  }

  Future<int> updateRecurringBill(Map<String, dynamic> row) async {
    final db = await database;
    return db.update('recurring_bills', row,
